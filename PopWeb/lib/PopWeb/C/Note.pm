...
PopWeb::C::Note - Scaffolding Controller Component
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    PopWeb::M::CDBI::Note->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
	...
	PopWeb::M::CDBI::Note->create_from_form( $c->form );
...
sub do_edit : Local {
	...
	PopWeb::M::CDBI::Note->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = PopWeb::M::CDBI::Note->retrieve($id);
