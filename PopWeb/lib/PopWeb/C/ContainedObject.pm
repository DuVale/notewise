package PopWeb::C::ContainedObject;
...
PopWeb::C::ContainedObject - Scaffolding Controller Component
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    PopWeb::M::CDBI::ContainedObject->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
	...
	PopWeb::M::CDBI::ContainedObject->create_from_form( $c->form );
...
sub do_edit : Local {
	...
	PopWeb::M::CDBI::ContainedObject->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = PopWeb::M::CDBI::ContainedObject->retrieve($id);
