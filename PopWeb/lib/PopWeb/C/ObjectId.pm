...
PopWeb::C::ObjectId - Scaffolding Controller Component
...
sub add : Local {
    my ( $self, $c ) = @_;
    $c->stash->{template} = 'ObjectId/add.tt';
}
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    PopWeb::M::CDBI::ObjectId->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
    my ( $self, $c ) = @_;
    $c->form( optional => [ PopWeb::M::CDBI::ObjectId->columns ] );
	...
	PopWeb::M::CDBI::ObjectId->create_from_form( $c->form );
...
sub do_edit : Local {
	...
	PopWeb::M::CDBI::ObjectId->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = PopWeb::M::CDBI::ObjectId->retrieve($id);
...
sub list : Local {
    my ( $self, $c ) = @_;
    $c->stash->{template} = 'ObjectId/list.tt';
}
