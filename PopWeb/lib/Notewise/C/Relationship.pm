package Notewise::C::Relationship;
...
Notewise::C::Relationship - Scaffolding Controller Component
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    Notewise::M::CDBI::Relationship->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
	...
	Notewise::M::CDBI::Relationship->create_from_form( $c->form );
...
sub do_edit : Local {
	...
	Notewise::M::CDBI::Relationship->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = Notewise::M::CDBI::Relationship->retrieve($id);
