...
Notewise::C::Note - Scaffolding Controller Component
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    Notewise::M::CDBI::Note->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
	...
	Notewise::M::CDBI::Note->create_from_form( $c->form );
...
sub do_edit : Local {
	...
	Notewise::M::CDBI::Note->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = Notewise::M::CDBI::Note->retrieve($id);
