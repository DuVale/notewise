...
Notewise::C::ObjectId - Scaffolding Controller Component
...
sub destroy : Local {
    my ( $self, $c, $id ) = @_;
    Notewise::M::CDBI::ObjectId->retrieve($id)->delete;
    $c->forward('list');
}
...
sub do_add : Local {
    my ( $self, $c ) = @_;
    $c->form( optional => [ Notewise::M::CDBI::ObjectId->columns ] );
...
sub do_edit : Local {
	...
	Notewise::M::CDBI::ObjectId->retrieve($id)->update_from_form( $c->form );
...
sub edit : Local {
    my ( $self, $c, $id ) = @_;
    $c->stash->{item} = Notewise::M::CDBI::ObjectId->retrieve($id);
